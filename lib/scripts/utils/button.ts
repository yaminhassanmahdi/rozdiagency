export const initButtonAnimation = () => {
  ...
  buttonWrappers.forEach((el) => {
    const buttonWrapper = el as HTMLElement;
    const iconWrapper = buttonWrapper.querySelector('.button-icon') as HTMLElement;
    const buttonText = buttonWrapper.querySelector('.button-text') as HTMLElement;
